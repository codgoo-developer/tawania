  ...
  return (
    <section id="strategic-goals-section" className="py-20 sm:py-28 bg-[#F6F7F2] relative">
      <div className="max-w-[1440px] mx-auto px-6 sm:px-8 lg:px-12">
        {/* Section Header with Line Badge matching original design */}
        <div className="flex items-center justify-end rtl:justify-start gap-3 mb-12 text-start">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-[#0B4F26] tracking-tight">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 sm:gap-6">
          {goalsList.map((goal, idx) => {
            ...
            return (
              <div
                key={goal.id || idx}
                className="bg-white rounded-2xl p-6 sm:p-7 border border-[#0B4F26]/5 shadow-xs hover:shadow-lg hover:border-[#0B4F26]/30 hover:-translate-y-1 transition-all duration-300 flex flex-col items-center text-center group min-h-[220px]"
              >
                ...
                <div className="flex flex-col gap-2 flex-grow justify-start w-full">
                  <h3 className="text-base sm:text-lg font-bold text-[#0B4F26] group-hover:text-[#0B4F26] transition-colors leading-snug">
                    {title}
                  </h3>
                  {desc && (
